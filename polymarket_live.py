...
def execute_trade(positions: Dict, market: Dict, signal: str, strategy: str) -> bool:
    ...
    try:
        ...
        print(f"✅ BUY: {market.get('question', 'Unknown')} @ {yes_price:.3f}, {shares:.1f} shares, ${position_amount:.2f} ({strategy})")
        return True
    ...
    try:
        ...
        for i, position in enumerate(positions["positions"]):
            if position["market_id"] == market.get("id"):
                ...
                print(f"{pnl_emoji} SELL: {market.get('question', 'Unknown')} @ {yes_price:.3f}, P&L: ${realized_pnl:.2f} ({position['unrealized_pnl_pct']:.2%}) [{exit_reason}]")
